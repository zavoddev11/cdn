let API_URL = `${window.location.protocol}//${window.location.hostname}:5459`;
...
function ChatWidget() {
  ...
  useEffect(() => {
    socket.on("message:receive", (message) => {
      ...
      if (message) {
        console.log({ message, localID: localStorage.getItem("chat__session_id") })
        if (message.data.session_id == localStorage.getItem("chat__session_id")) {
    ...
    <div className="ai_chatbox">
      ...
      {isOpen && (
        <div className=" chat-widget">
          ...
          <div style={{ paddingTop: "5px", marginBottom: "8px" }} className="power-by">Powered by <span>Zavod-it.com</span></div>
